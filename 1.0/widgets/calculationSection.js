define([
	...
	return declare(null, {
		...
		constructor: function(params) {
			...
			this.getLayersService = params.services.getLayersService;
		...
		postCreate: function() {
			...
			on(dojo.byId("bspiModeButton"), "click", lang.hitch(this, function() {
				...
				document.getElementById("calculateBspiButton").style.display = "block";
			}));
		...
		getLayers: function() {
			...
			/*var gp = new Geoprocessor(this.getLayersService);
			var params = {};
			gp.submitJob(params, gpJobComplete, gpJobStatus, gpJobFailed);
			var that = this;

			function gpJobComplete(jobinfo) {
				that.layersReceived = true;
				gp.getResultData(jobinfo.jobId, "output", function(data) {
					console.log("getLayers", data);
					var ecLayers = [], plLayers = [];
					for (i=0, len = data.value.ecKeys.length; i < len; ++i) {
		    		ecLayers.push({"value": data.value.ecKeys[i], "label": data.value.ecKeys[i] + ": " + data.value.ecLabels[i]});
					}
					for (i=0, len = data.value.plKeys.length; i < len; ++i) {
		    		plLayers.push({"value": data.value.plKeys[i], "label": data.value.plKeys[i] + ": " + data.value.plLabels[i]});
					}
					that.setupSelectionContainer("EC", ecLayers);
					that.setupSelectionContainer("PL", plLayers);
					document.getElementById("loadingCover").style.display = "none";
	      });
			}
		...
		createSSGrid: function() {
			console.log(this.ecLayersForTooltip);
			console.log(this.plLayersForTooltip);
			document.getElementById("loadingCover").style.display = "block";
			var layout = [
				{
					noscroll: true,
    	...
    	this.grid.startup();
			...
			fetch(windowUrl + appVersion + "/config/sscores.json")
				...
				.then(lang.hitch(this, function(data) {
					...
					document.getElementById("calculateBsiiButton").style.display = "block";
					document.getElementById("loadingCover").style.display = "none";
				}))
				.catch(lang.hitch(this, function(error) {
					document.getElementById("loadingCover").style.display = "none";
					console.log(error);
					alert("Unable to get sensitivity scores. Try to reload page. Contact administrator at data@helcom.fi if alert appears again.");
				}));
		},
